...
public class NodeConnector {
    ...
    private void connectEdgeToNodes(final String connectorTypeId,
        ...
        if (Objects.nonNull(jsiDMNElementReference)) {
            ...
            final Optional<JSIDMNEdge> dmnEdge = edges.stream()
                    .filter(e -> {
            ...
            final JSIDMNEdge obj = Js.uncheckedCast(dmnEdge.get());
            final Node requiredNode = getNode(obj, nodeEntries);
            final String id = obj.getDmnElementRef().getLocalPart();
            // build id private static String idOfDMNorWBUUID(final org.kie.dmn.model.api.DMNElement dmn) {
            //        return dmn.getId() != null ? dmn.getId() : UUID.uuid();
            //    }
            final Edge myEdge = factoryManager.newElement(diagramId + "#" + id,// or build id
...
//            if (dmnEdge.isPresent()) {
            final JSIDMNEdge jsidmnEdge = Js.uncheckedCast(dmnEdge.get());
            setConnectionMagnets(myEdge, connectionContent, jsidmnEdge);
//            } else {
//                setConnectionMagnets(myEdge, connectionContent);
//            }
        }
    }
    ...
    private void setConnectionMagnets(final Edge edge, final ViewConnector connectionContent) {
    ...
    private void setConnectionMagnets(final Edge edge, final ViewConnector connectionContent, final JSIDMNEdge jsidmnEdge) {
    ...
    private Node getNode(final JSIDMNEdge jsidmnEdge,
        ...
        final Point2D nearest = Collections.min(entriesByPoint2D.keySet(), (p1, p2) -> {
            final Double d1 = source.distance(p1);
            final Double d2 = source.distance(p2);
